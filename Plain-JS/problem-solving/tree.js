...
const preOrderTraversal2 = function (root, indentation) {
  if (root.items.length === 0) {
    console.log(`${indentation}${root.name}`);
    return;
  }
  ...
  root.items.forEach((item) => {
    preOrderTraversal2(item, indentation + "  ");
  });
};

const preOrderTraversal3 = function (root, target, newObj) {
  if (root.items.length === 0) {
    return;
  }

  if (root.id === target && root.isFolder) {
    root.items.push(newObj);
    return;
  }

  root.items.forEach((item) => {
    preOrderTraversal3(item, target, newObj);
  });
};

preOrderTraversal3(explorer, "7", {
  id: 100,
  name: "module.tsx",
  isFolder: false,
  items: [],
